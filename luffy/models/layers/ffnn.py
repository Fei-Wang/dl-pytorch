...
class FFNN(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers=2, dropout=0., act_fct=nn.GELU):
        ...
        if num_layers == 1:
            self.net.append(nn.Sequential(nn.Linear(input_dim, output_dim), nn.Dropout(dropout)))
        else:
            ...
            self.net.append(nn.Sequential(nn.Linear(hidden_dim, output_dim), nn.Dropout(dropout)))
